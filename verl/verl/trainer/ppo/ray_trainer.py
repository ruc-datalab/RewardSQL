...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_grpo_process_advantage_avg_unique(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index)
...
def compute_data_metrics(batch, use_critic=True):
    ...
    # if 'outcome_reward' in batch.batch.keys():
    #     outcome_reward = batch.batch['outcome_reward']
    # else:
    #     outcome_reward = None
    ...
    metrics = {
        ...
        #     torch.mean(outcome_reward).detach().item(),
        'critic/score/mean':
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor, _ = self.val_reward_fn.verify(test_batch, eval_mode=True)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.data_logger is not None:
                            self.data_logger.log_step(self.global_steps, batch, metrics)
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                        ...
                        with _timer('testing', timing_raw):
                            ...
                            pprint(f'{self.global_steps} steps validation metrics: {val_metrics}')
                            logger.log(data=val_metrics, step=self.global_steps)
                            if is_last_step:
