...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, global_step: int, remove_previous_ckpt=True, *args, **kwargs):
        ...
        if self.rank == 0:
            ...
            # 只保存配置和tokenizer，避免内存爆炸
            # 注意：这里使用processing_class替代tokenizer，与init保持一致
            self.model._fsdp_wrapped_module.config.save_pretrained(hf_local_path)
            self.processing_class.save_pretrained(hf_local_path)
            print(f"[rank-{self.rank}]: 配置和tokenizer已保存到 {hf_local_path}")
            print(f"[rank-{self.rank}]: 要获取完整模型，请使用model_merger.py脚本合并分片模型")
